document.addEventListener('DOMContentLoaded', () => {
    ...
    const atualizarSaldo = (novoSaldo, novoLimite) => { 
        ...
        ultimoLimiteSaqueSC = parseFloat(novoLimite || 0); 
        atualizarUIValores();
    };
                ...
                try {
                    ...
                    setTimeout(() => {
                        if (window.tronWeb && window.tronWeb.defaultAddress.base58) {
                            if(tronStatus) tronStatus.textContent = "✅ Conectado: " + window.tronWeb.defaultAddress.base58;
                            btnConnectTron.style.display = 'none';
                            if(btnClaimTron) btnClaimTron.style.display = 'block';
                        } else {
    ...
    if (btnClaimTron) {
        btnClaimTron.addEventListener('click', async () => {
            if (!window.tronWeb || !window.tronWeb.defaultAddress.base58) return alert("TronLink desconectado!");

            try {
                if(tronStatus) tronStatus.textContent = "⏳ Aguardando confirmação na TronLink...";
    ...
    const atualizarCalculoPix = () => {
        ...
        if(pixTaxaSc) pixTaxaSc.textContent = taxa.toFixed(2);
        if(pixReceberBrl) pixReceberBrl.textContent = (liquido / scRate).toFixed(2);
    };
    ...
    const saquePixForm = document.getElementById('saque-pix-form');
    if (saquePixForm) {
        saquePixForm.addEventListener('submit', async (e) => {
            e.preventDefault();
        ...
        socioPagamento.addEventListener('change', async () => {
            ...
            if (metodo === 'Pix') {
                ...
                try {
                    ...
                    if (data.sucesso) {
                        textoEl.textContent = "Escaneie o QR Code abaixo ou use o Copia e Cola para pagar:";
                        dadoEl.innerHTML = `<img src="${data.imagemQrcode}" style="max-width:200px; margin-top:10px; border-radius:8px;"><br><textarea readonly style="width:100%; height:60px; margin-top:10px; background:#222; color:#fff; border:1px solid #d4af37; padding:5px; border-radius:5px;">${data.pixCopiaECola}</textarea>`;
                    } else {
                        textoEl.textContent = "Erro ao gerar o Pix. Verifique a configuração da Efí.";
                    }
                } catch (e) {
                    textoEl.textContent = "Erro de conexão com o servidor.";
                }
    ...
    const atualizarUIStaking = (usuario) => {
        if(stakedAmountEl) stakedAmountEl.textContent = parseFloat(usuario.stakedAmount || 0).toFixed(2);
        if (usuario.canUnstakeAt && new Date() < new Date(usuario.canUnstakeAt)) {
            if(unstakeDateEl) unstakeDateEl.textContent = new Date(usuario.canUnstakeAt).toLocaleString('pt-BR');
            if(unstakeBtn) { unstakeBtn.disabled = true; unstakeBtn.style.cursor = 'not-allowed'; }
        } else {
            if(unstakeDateEl) unstakeDateEl.textContent = "Agora";
            const temStaking = (usuario.stakedAmount || 0) > 0;
            if(unstakeBtn) { unstakeBtn.disabled = !temStaking; unstakeBtn.style.cursor = temStaking ? 'pointer' : 'not-allowed'; }
        }
    };
        ...
        try {
            ...
            if (data.sucesso) {
                ...
                if (!isUpdate) { 
                    ...
                    if(document.getElementById('solana-wallet')) document.getElementById('solana-wallet').value = data.usuario.solanaWallet || '';
                    if(document.getElementById('tron-wallet')) document.getElementById('tron-wallet').value = data.usuario.tronWallet || '';
                    if (data.usuario.isAdmin && adminBtn) { adminBtn.style.display = 'inline-block'; }
                    ...
                    if(categoriasContainer && marketplaceListaEl) {
                ...
                if(document.getElementById('status-socio')) {
                ...
                if(data.usuario.vencimentoSocio && document.getElementById('vencimento-socio-box')) {
    ...
    if(saqueForm) {
        saqueForm.addEventListener('submit', async (e) => {
            ...
            const data = await response.json(); alert(data.mensagem);
            if (data.sucesso) { saqueForm.reset(); carregarHistoricoSaques(); carregarDashboard(true); }
        });
    }
    ...
    if(transferirForm) {
        transferirForm.addEventListener('submit', async (e) => {
            ...
            const data = await response.json(); alert(data.mensagem);
            if (data.sucesso) { atualizarSaldo(data.novoSaldo, ultimoLimiteSaqueSC); carregarExtrato(); transferirForm.reset(); }
        });
    }

    const marketplaceLista = document.getElementById('marketplace-lista');
    if(marketplaceLista) {
        marketplaceLista.addEventListener('click', async (e) => {
            if (e.target.classList.contains('comprar-btn')) {
                ...
                const data = await response.json(); alert(data.mensagem);
                if(data.sucesso) { atualizarSaldo(data.novoSaldo, ultimoLimiteSaqueSC); carregarExtrato(); }
            }
        });
    }
    ...
    if(stakeForm) {
        stakeForm.addEventListener('submit', async (e) => {
            ...
            const data = await response.json(); alert(data.mensagem);
            if (data.sucesso) { atualizarSaldo(data.usuario.saldo, ultimoLimiteSaqueSC); atualizarUIStaking(data.usuario); stakeForm.reset(); }
        });
    }
    ...
    if (giftForm) {
        ...
        giftForm.addEventListener('submit', async (e) => {
            ...
            const data = await res.json(); alert(data.mensagem);
            if(data.sucesso){ carregarDashboard(true); giftForm.reset(); updateG(); }
        });
    }
    ...
    if(resgatarGiftForm) {
        resgatarGiftForm.addEventListener('submit', async (e) => {
            ...
            const data = await res.json(); alert(data.mensagem);
            if(data.sucesso) { carregarDashboard(true); resgatarGiftForm.reset(); }
        });
    }
    ...
    if (rechForm) {
        ...
        rechForm.addEventListener('submit', async (e) => {
            ...
            const data = await res.json(); alert(data.mensagem);
            if(data.sucesso){ carregarDashboard(true); rechForm.reset(); updateR(); }
        });
    }

    if (logoutBtn) logoutBtn.addEventListener('click', async () => { await fetch('/logout', { method: 'POST' }); window.location.href = '/index.html'; });
